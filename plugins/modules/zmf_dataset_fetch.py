...
message:
    ...
    sample:
        ...
        sample2: "The matched contents in the data set ZOSMF.ANSIBLE.SAMPLE(MEMBER) is fetched successfully and saved in: /tmp/dataset/ZOSMF.ANSIBLE.SAMPLE/MEMBER.serarch."
        ...
        sample4: "A range of records in the data set ZOSMF.ANSIBLE.SAMPLE(MEMBER) is fetched successfully and saved in: /tmp/dataset/ZOSMF.ANSIBLE.SAMPLE/MEMBER.range."
...
dataset_matched_range:
    description:
        ...
        - Return I(dataset_matched_range=p,q), where I(p) is the first matched line in the USS file and I(q) is the number of lines returned.
    returned: on success when I(dataset_data_type=text) and I(dataset_search) is specified
    type: str
    sample: "0,500"
dataset_checksum:
...
def validate_module_params(module):
    ...
    if 'dataset_search' in params and params['dataset_search'] is not None:
        if isinstance(params['dataset_search'], dict):
            ...
            if (not 'keyword' in search or search['keyword'].strip() == ''):
            # if search['keyword'] is None or search['keyword'].strip() == '':
                module.fail_json(msg='Invalid variable: dataset_search. Missing required suboption or invalid suboption: keyword.')
        else:
            module.fail_json(msg='Invalid variable: dataset_search. It should be a dict and contain the following suboptions only: ' 
    ...
    if 'dataset_range' in params and params['dataset_range'] is not None:
        if isinstance(params['dataset_range'], dict):
            ...
            module.fail_json(msg='Invalid variable: dataset_range. It should be a dict and contain the following suboptions only: ' 
...
def fetch_dataset(module):
    ...
    if status_code == 404:
        ...
        if status_code == 304:
            fetch_result['message'] = 'The data set is not fetched because the checksum of source is matched with the supplied one.'
            ...
            if response_headers['Content-Type'] is not None:
                ...
                if 'X-IBM-Record-Range' in response_headers:    
                    ...
                    if response_fetch.text != '':
                        ...
                        fetch_result['message'] = 'The matched content in the data set ' + dataset + 'is fetched successfully. The search result is returned in' \
                        + '"dataset_matched_content" and saved in "' + path + save_file + '.search."'
                    else:
                        fetch_result['dataset_matched_content'] = []
                        fetch_result['message'] = 'The data set ' + dataset + ' is not fetched since no matched contents is found with the specified search keyword."'
