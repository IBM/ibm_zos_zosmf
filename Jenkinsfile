...
pipeline {
	...
	stages {
		...
		script {
			    ...
			    dir("${remoteWorkspace}") {
				        if (fileExists('ibm-ibm_zos_zosmf-3.0.0.tar.gz')) {
					            echo "ibm-ibm_zos_zosmf-3.0.0.tar.gz existed"
					            sh 'rm ibm-ibm_zos_zosmf-3.0.0.tar.gz'
					            sh '/usr/local/bin/ansible-galaxy collection build'
				        } else {
					            sh '/usr/local/bin/ansible-galaxy collection build'
				        }
				        sh "pwd"
				        sh '/usr/local/bin/ansible-galaxy collection install ibm-ibm_zos_zosmf-3.0.0.tar.gz'
			    }
		    }
        }
    }
    ...
    stage('Test') {
        ...
        steps {
		    ...
		    echo 'DatasetAPI Create/Rename/Delete/Remote Copy BVT'
		    dir("/Users/strangepear2019/.ansible/collections/ansible_collections/ibm/ibm_zos_zosmf/tests/CICD/playbooks") {
