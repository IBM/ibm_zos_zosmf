...
pipeline {
	...
	steps {
		...
		script {
			...
			dir("${remoteWorkspace}") {
				if (fileExists('ibm-ibm_zos_zosmf-2.1.1.tar.gz')) {
					echo "ibm-ibm_zos_zosmf-2.1.1.tar.gz existed"
					sh 'rm ibm-ibm_zos_zosmf-2.1.1.tar.gz'
					sh '/usr/local/bin/ansible-galaxy collection build'
				} else {
					sh '/usr/local/bin/ansible-galaxy collection build'
				}
				sh "pwd"
				sh '/usr/local/bin/ansible-galaxy collection install ibm-ibm_zos_zosmf-2.1.1.tar.gz'
			}
		}
            }
        }
